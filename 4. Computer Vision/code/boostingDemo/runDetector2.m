...
% Loop on test images  NtestImages
for i = 1:N_test_precomputed
...
%     imshow(newimg)
    %Loop with different configurations (if we want to compare setups using different numbers of weak classifiers)
    for m = 1:length(NweakClassifiers)
        ...
        rows = size(Img,1);
        cols = size(Img,2);
        [Score, boundingBox, boxScores] = singleScaleBoostedDetector2(i, data,FeaturesTest_img, rows, NweakClassifiers(m));
        
        % Evaluate performace looking at precision-recall with different
        % thresholds over the overlapping between bounding boxes (detected and ground truth):
    ...
    %Computing precision and recall  by accumulating the results of each
    %image => We have vectors of prec and recall for different THs on the
    %overlapping
